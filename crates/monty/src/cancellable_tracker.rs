...
//! next `check_time`, `on_allocate`, or `on_grow` call the tracker will return a
...
///
/// On each `check_time`, `on_allocate`, and `on_grow` call the atomic flag is
/// inspected first. If set, a `ResourceError::Exception` carrying a
/// `KeyboardInterrupt` is returned immediately — the same exception CPython
/// raises on Ctrl+C. Because resource errors (other than `RecursionError`) are
/// converted to uncatchable exceptions by the VM, Python code cannot suppress
/// the cancellation with a bare `except:`.
///
/// # Thread safety
///
...
mod tests {
    ...
    #[test]
    fn cancel_flag_stops_on_grow() {
        let tracker = CancellableTracker::new(NoLimitTracker);
        let flag = tracker.cancel_flag();

        flag.store(true, Ordering::Relaxed);

        let err = tracker.on_grow(128).unwrap_err();
        assert!(matches!(err, ResourceError::Exception(ref e) if e.exc_type() == ExcType::KeyboardInterrupt));
    }
    ...
    fn inner_limits_still_enforced() {
        ...
        let tracker = CancellableTracker::new(LimitedTracker::new(limits));
