...
results
";
    let runner = MontyRun::new(code.to_owned(), "test.py", vec![], vec!["async_call".to_owned()]).unwrap();
    ...
    let results = vec![(calls[1].0, ExternalResult::Return(MontyObject::Int(5)))];
    let progress = state.resume(results, &mut PrintWriter::Stdout).unwrap();
...
";
    ...
    let results = vec![(call_ids[0], ExternalResult::Return(MontyObject::Int(999)))];
    let progress = state.resume(results, &mut PrintWriter::Stdout).unwrap();
    ...
    let results = vec![(remaining[0], ExternalResult::Return(MontyObject::Int(42)))];
    let progress = state.resume(results, &mut PrintWriter::Stdout).unwrap();
    ...
    let results = vec![(last_id, ExternalResult::Return(MontyObject::Int(42)))];
    let progress = state.resume(results, &mut PrintWriter::Stdout).unwrap();
